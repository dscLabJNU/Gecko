#ifndef __TIMEDTSMPED_FFIBA_H__
#define __TIMEDTSMPED_FFIBA_H__
...
namespace TimeStampedFFiBA
{
  ...
  private:
    ...
    //以下操作都是对于当前节点进行计算的，而不是整棵树 根据节点状态计算当前节点总agg
  ...
  public:
    ...
    //重新计算当前节点agg 若新结果与旧结果不一致 返回true
  ...
  //Node方法结束
    ...
    class leftAggBTree{
      binOpFunc _binOp;
      aggT _identE;
      deque<Node *> _freeList; // 释放节点缓存
      Node *_root;
      Node *_leftFinger, *_rightFinger; // 指针
      size_t _size;
  ...
  class ConsolidatedTreelet {
    ...
    size_t size() {return commonRoots.size();}
  ...
  void doInitialMultisearch(pair<timeT, aggT> *entries, size_t _tcc, size_t _tcp,
                            ConsolidatedTreelet &treelets) {
    std::deque<ipathBreadcrumb> latestPath;
    bool first = true;
    timeT prevTime;
    int level = 0;
    for (int it = 0; it != _tcp; it++) {
      const auto [time, value] = entries[_tcc+it];
      auto [node, found, thisLevel] = multiSearchFind(latestPath, time, level);
      if (found) {
        node->localInsertEntry(_binOp, time, _binOp.lift(value));
        if (first || prevTime != time) // a new (i.e., unseen) timestamp
  ...
  leftAggBTree(binOpFunc binOp_, aggT identE_)
        : _binOp(binOp_),
          _identE(identE_),
          _freeList(),
          _root(newNode(true)),
    ...
    void createTree(pair<timeT,aggT> *pValue, size_t _tcc, size_t _tcp){

      // bulkInsertForEmpty(pValue,_tcc,_tcp);
      for(int i=0; i<_tcp; i++){
        insert_lifted(pValue[_tcc+i].first,pValue[_tcc+i].second);
        // cout<<"正在插入节点，value：" << pValue[_tcc+i].second <<endl;
      }


      
      // Node* tempNode = _root;
      // recalAllAggsUp(_root);
      // while(!tempNode->isLeaf()){
      //   tempNode = tempNode->getChild(0);
      //   recalAllAggsUp(tempNode);
      // }
      // tempNode->localRepairAgg(_binOp);


      // cout << "建树成功:" << query() <<"!!!"<<endl;
      // vector<pair<timeT,aggT>> vec();
      // bulkInsert(vec);
    }
  ...
  void bulkInsertForEmpty(pair<timeT, aggT> *entries,size_t _tcc, size_t _tcp){
    ...
    doInitialMultisearch(entries, _tcc, _tcp, curTreelets);
    _size += curTreelets.commonRoots[0].members.size();
  ...
  aggT query() const {
    if (kind==finger) {
      if (_root->isLeaf()) {
        ...
        IF_COLLECT_STATS(++statsCombineCount);
        IF_COLLECT_STATS(++statsCombineCount);
        return _binOp.combine(leftAgg,_binOp.combine(rootAgg, rightAgg));
      }
    } else {
      return _root->getAgg();
    }
  ...
  friend inline std::ostream& operator<<(std::ostream& os, Aggregate const& x) {
    ...
    public:
    typedef leftAggBTree* latP;
    Aggregate(binOpFunc binOp_, aggT identE_, timeD size_)
      :  _binOp(binOp_) ,
         _identE(identE_) ,
         sz(0) , max_sz(computeMaxSz(size_)),
         k(computeK(max_sz)) , 
         block(max_sz/k),
         _cc(0),_cp(0),
         _tcc(0),_tcp(0),
         ...
         full(false)
         {
            // std::allocator<leftAggBTree> alloc;
            tValue = new latP[k];

            for(int i = 0; i < k; i++){
                tValue[i] = new leftAggBTree(binOp_, identE_);
            }
         }
    ...
    size_t computeMaxSz(timeD size){
      return size.count();
    ...
    void insert(timeT const& time, inT const& value){
      ...
      ++sz;
      aggT lifted = _binOp.lift(value);
      size_t _ncp;
      if(time >= startTime){
        _cc = std::chrono::duration_cast<std::chrono::milliseconds>(time - startTime).count() / block + _tcc;
        if(_cc == k){
          _cc = k-1;
        }else{
          _cc %= k; 
    ...
    void evict(){
      ...
      while (tValue[_tcc]->isEmpty())
      {
        _tcc = (_tcc + 1) % k;
        gValue = _identE;
        for (size_t i = 0; i < k; i++)
        {
          gValue = _binOp.combine(tValue[_tcc]->query(), gValue);
        }
      }
      
      if(!tValue[_tcc]->isEmpty()){
        startTime = tValue[_tcc]->oldest();
        tValue[_tcc]->evict();
        --sz;
      }
    }
    ...
    outT query() {
      return _binOp.lower(_binOp.combine(tValue[_tcc]->query(), gValue));
      
      // return _binOp.lower(gValue);
      // return _identE;
    }
  ...
  template <typename timeT, class BinaryFunction,int minArity , Kind kind,class T>
  Aggregate<BinaryFunction, timeT, minArity, kind> make_aggregate(BinaryFunction f, T elem, size_t size)
  {
    return Aggregate<BinaryFunction, timeT, minArity, finger>(f, elem, size);
  }

  template <typename timeT, class BinaryFunction,int minArity , Kind kind,class T>
  Aggregate<BinaryFunction, timeT, minArity, kind> make_aggregate(BinaryFunction f, T elem, timeD size)
  {
    return Aggregate<BinaryFunction, timeT, minArity, finger>(f, elem, size);
  }
  ...
  {
    template <typename T>
    Aggregate<BinaryFunction, timeT, minArity, kind> operator()(T elem, size_t size)
    {
      BinaryFunction f;
      return make_aggregate<timeT, BinaryFunction, minArity, kind>(f, elem, size);
    }

    template <typename T>
    Aggregate<BinaryFunction, timeT, minArity, kind> operator()(T elem, timeD size)
    {
      BinaryFunction f;
      return make_aggregate<timeT, BinaryFunction, minArity, kind>(f, elem, size);
    }
