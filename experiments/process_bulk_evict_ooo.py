    ...
    for f, windows in func_to_data.items():
        for w, aggs in windows.items():
            ...
            make_throughput_graph(
                preamble, f, w, aggs, constant="window", varying="ooo distance"
            )
    functions = ["sum", "geomean", "bloom"]
    aggs_array = [func_to_data[f][ww] for f in functions] 
    make_throughput_panel(preamble, functions, ww, aggs_array, constant="window", varying="ooo distance")
...
def func_varying_ooo_distance(agg_to_func):
    ret = collections.defaultdict(
        lambda: collections.defaultdict(lambda: collections.defaultdict(dict))
    )
    for agg, func in agg_to_func.items():
...
def main():
    ...
    make_bulk_size_varying_graph("bulk_evict", func_varying_ooo_distance(agg_to_func))
